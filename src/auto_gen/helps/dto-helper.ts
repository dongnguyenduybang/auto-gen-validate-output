  ...
  if (decorators['type']) {
    if (decorators['type'] === 'string') {
      ...
      if (decorators['enumType']) {
        ...
        const numberValues = enumValues.filter((val) => typeof val === 'number');
        const uniqueNumberValues = [...new Set(numberValues)];
        if (!uniqueNumberValues.includes(value)) {
          errors.push(
            `${field} ${ErrorMessage.INVALID_RANGE_NUMBER} ${uniqueNumberValues.join(', ')}`,
          );
