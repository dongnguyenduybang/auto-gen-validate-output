...
function processFiles() {
    ...
    Object.entries(fileMap).forEach(([className, { dtoPath, requestPath }]) => {
        ...
        try {
            ...
            if (typeof dtoClass !== 'function' || !/^\s*class\s/.test(dtoClass.toString())) {
            ...
            fs.writeFileSync(outputFilePath, JSON.stringify(testCasePayload, null, 4), 'utf-8');
...
function groupFilesByName(files: string[]): Record<string, { dtoPath?: string; requestPath?: string }> {
