...
export async function validateSendStickerMessage(instance: any, payload: any): Promise<string[]> {
    const errors: string[] = [];
        ...
        for (const key of keys) {
            ...
            if (decorators?.type === 'array' && Array.isArray(valueResponse)) {
                for (const [index, item] of valueResponse.entries()) {
                    ...
                    await validateObject(item, itemPrototype, `${field}[${index}]`);
                }
            }
            ...
            //check obj
                ...
                if (key === 'sticker') {
                    ...
                    await validateObject(stickerInstance, nestedPrototype, field);
                } else {
                    ...
                    if (decorators?.validIf) {
                        ...
                        if (condition === 'attachmentType') {
                            if (valueResponse !== condition2) {
                                errors.push(`${field} must be equal with value ${condition2} `);
                            }
                        }
                        ...
                        if (condition === 'collectionId') {
                            ...
                            if (collectionId === collectionSticker.collectionId) {
                                errors.push(`${field} is not found with collectionId ${collectionId}`);
                            }
