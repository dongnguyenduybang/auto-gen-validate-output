...
import { getConfig, setupConfiguration } from '../utils/get-config';
...
export function getDtoFolderPath(dtoName: string): string | null {
  ...
  const testRequestDir = getConfig('testRequestsDir') as string;
  const testRequestPath = path.resolve(workingDir, testRequestDir);
  ...
  return searchDir(testRequestPath);
}
