    ...
    import { UnsubscribeAllRequest } from '././unsubscribe-all.request';
    describe('Testcase for unsubscribe-all', () => {
        let totalTests = 0;
        let allSteps = [];
        let failedTests: any[] = [];
        let codedTest: any[] = [];
        ...
        beforeAll(async () => {
          ...
          const beforeAllSteps = UnsubscribeAllRequest.options
        ...
        beforeEach(async () => {
          testCaseNumber++;
          const beforeEachSteps = UnsubscribeAllRequest.options
      ...
      afterEach(async () => {
          testCaseNumber++;
          const afterEachSteps = UnsubscribeAllRequest.options
         ...
         afterAll(async () => {
          const afterAllSteps = UnsubscribeAllRequest.options
          ...
          const testResult: TestResult = {
            path: '/Notification/UnsubscribeAll',
  ...
  const fileName = 'unsubscribe-all' + (chunkNumber ? `-chunk-undefined` : '') + '.result.json';
  const filePath = path.join(reportDir, fileName);
  fs.writeFileSync(filePath, JSON.stringify(testResult, null, 2), 'utf-8');

  console.log(`📝 Saved result for unsubscribe-all chunk single to ${filePath}`);
    });
        })
