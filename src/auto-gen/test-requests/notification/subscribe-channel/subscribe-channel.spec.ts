    ...
    import { SubscribeChannelRequest } from '././subscribe-channel.request';
    describe('Testcase for subscribe-channel', () => {
        let totalTests = 0;
        let allSteps = [];
        let failedTests: any[] = [];
        let codedTest: any[] = [];
        ...
        beforeAll(async () => {
          ...
          const beforeAllSteps = SubscribeChannelRequest.options
        ...
        beforeEach(async () => {
          testCaseNumber++;
          const beforeEachSteps = SubscribeChannelRequest.options
      ...
      afterEach(async () => {
          testCaseNumber++;
          const afterEachSteps = SubscribeChannelRequest.options
         ...
         afterAll(async () => {
          const afterAllSteps = SubscribeChannelRequest.options
  ...
  const fileName = 'subscribe-channel' + (chunkNumber ? `-chunk-undefined` : '') + '.result.json';
  const filePath = path.join(reportDir, fileName);
  fs.writeFileSync(filePath, JSON.stringify(testResult, null, 2), 'utf-8');

  console.log(`📝 Saved result for subscribe-channel chunk single to ${filePath}`);
    });
        })
