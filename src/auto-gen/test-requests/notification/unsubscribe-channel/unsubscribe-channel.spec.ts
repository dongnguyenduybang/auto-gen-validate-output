    ...
    import { UnsubscribeChannelRequest } from '././unsubscribe-channel.request';
    describe('Testcase for unsubscribe-channel', () => {
        let totalTests = 0;
        let allSteps = [];
        let failedTests: any[] = [];
        let codedTest: any[] = [];
        ...
        beforeAll(async () => {
          ...
          const beforeAllSteps = UnsubscribeChannelRequest.options
        ...
        beforeEach(async () => {
          testCaseNumber++;
          const beforeEachSteps = UnsubscribeChannelRequest.options
      ...
      afterEach(async () => {
          testCaseNumber++;
          const afterEachSteps = UnsubscribeChannelRequest.options
         ...
         afterAll(async () => {
          const afterAllSteps = UnsubscribeChannelRequest.options
          ...
          const testResult: TestResult = {
            path: '/Notification/UnsubscribeChannel',
            className: 'unsubscribe-channel',
            allSteps: allSteps,
  ...
  const fileName = 'unsubscribe-channel' + (chunkNumber ? `-chunk-undefined` : '') + '.result.json';
  const filePath = path.join(reportDir, fileName);
  fs.writeFileSync(filePath, JSON.stringify(testResult, null, 2), 'utf-8');

  console.log(`📝 Saved result for unsubscribe-channel chunk single to ${filePath}`);
    });
        })
