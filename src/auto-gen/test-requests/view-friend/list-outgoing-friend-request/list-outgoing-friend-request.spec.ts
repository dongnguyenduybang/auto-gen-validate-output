    ...
    import { ListOutgoingFriendRequestRequest } from '././list-outgoing-friend-request.request';
    describe('Testcase for list-outgoing-friend-request', () => {
        let totalTests = 0;
        let allSteps = [];
        let failedTests: any[] = [];
        let codedTest: any[] = [];
        ...
        beforeAll(async () => {
          ...
          const beforeAllSteps = ListOutgoingFriendRequestRequest.options
        ...
        beforeEach(async () => {
          testCaseNumber++;
          const beforeEachSteps = ListOutgoingFriendRequestRequest.options
      ...
      afterEach(async () => {
          testCaseNumber++;
          const afterEachSteps = ListOutgoingFriendRequestRequest.options
         ...
         afterAll(async () => {
          const afterAllSteps = ListOutgoingFriendRequestRequest.options
          ...
          const testResult: TestResult = {
            ...
            className: 'list-outgoing-friend-request',
            allSteps: allSteps,
  ...
  const fileName = 'list-outgoing-friend-request' + (chunkNumber ? `-chunk-undefined` : '') + '.result.json';
  const filePath = path.join(reportDir, fileName);
  fs.writeFileSync(filePath, JSON.stringify(testResult, null, 2), 'utf-8');

  console.log(`📝 Saved result for list-outgoing-friend-request chunk single to ${filePath}`);
    });
        })
