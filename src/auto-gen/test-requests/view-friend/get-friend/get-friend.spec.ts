    ...
    import { GetFriendRequest } from '././get-friend.request';
    describe('Testcase for get-friend', () => {
        let totalTests = 0;
        let allSteps = [];
        let failedTests: any[] = [];
        let codedTest: any[] = [];
        ...
        beforeAll(async () => {
          ...
          const beforeAllSteps = GetFriendRequest.options
        ...
        beforeEach(async () => {
          testCaseNumber++;
          const beforeEachSteps = GetFriendRequest.options
    ...
    try {
      ...
      afterEach(async () => {
          testCaseNumber++;
          const afterEachSteps = GetFriendRequest.options
         ...
         afterAll(async () => {
          const afterAllSteps = GetFriendRequest.options
  ...
  const fileName = 'get-friend' + (chunkNumber ? `-chunk-undefined` : '') + '.result.json';
  const filePath = path.join(reportDir, fileName);
  fs.writeFileSync(filePath, JSON.stringify(testResult, null, 2), 'utf-8');

  console.log(`📝 Saved result for get-friend chunk single to ${filePath}`);
    });
        })
