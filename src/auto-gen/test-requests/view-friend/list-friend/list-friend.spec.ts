    ...
    import { ListFriendRequest } from '././list-friend.request';
    describe('Testcase for list-friend', () => {
        let totalTests = 0;
        let allSteps = [];
        let failedTests: any[] = [];
        let codedTest: any[] = [];
        ...
        beforeAll(async () => {
          ...
          const beforeAllSteps = ListFriendRequest.options
        ...
        beforeEach(async () => {
          testCaseNumber++;
          const beforeEachSteps = ListFriendRequest.options
    ...
    try {
      ...
      afterEach(async () => {
          testCaseNumber++;
          const afterEachSteps = ListFriendRequest.options
         ...
         afterAll(async () => {
          const afterAllSteps = ListFriendRequest.options
  ...
  const fileName = 'list-friend' + (chunkNumber ? `-chunk-undefined` : '') + '.result.json';
  const filePath = path.join(reportDir, fileName);
  fs.writeFileSync(filePath, JSON.stringify(testResult, null, 2), 'utf-8');

  console.log(`📝 Saved result for list-friend chunk single to ${filePath}`);
    });
        })
