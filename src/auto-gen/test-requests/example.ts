import { IsString, IsDefined, IsEnum, IsInvalid, IsNotEmpty, IsULID, ValidIf, IsNotNull } from '../decorator';
...
function analyzeTestCases(testCases: any[], dtoClass: any) {
    ...
    singleErrorCases = testCases.filter(tc =>
        tc.expects.length === 1
    );

    pairwiseErrorCases = testCases.filter(tc =>
        tc.expects.length >= 2
    );
    ...
    console.log(`Pairwise coverage: ${coveredPairs}/${totalPossiblePairs} combinations`);
}
