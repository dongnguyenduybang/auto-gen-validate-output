    ...
    import { ListMediaRequest } from '././list-media.request';
    describe('Testcase for list-media', () => {
        let totalTests = 0;
        let allSteps = [];
        let failedTests: any[] = [];
        let codedTest: any[] = [];
        ...
        beforeAll(async () => {
          ...
          const beforeAllSteps = ListMediaRequest.options[0]?.beforeAll || [];
        ...
        beforeEach(async () => {
          testCaseNumber++;
          const beforeEachSteps = ListMediaRequest.options[0]?.beforeEach || [];
    ...
    try {
        const response = await resolveCallAPI(
            'listMedia',
            {"x-session-token":"{{token}}"},
            {"workspaceId":"0","channelId":"{{channelId}}","type":1,"limit":1,"nextPageToken":123},
            contextData
        );
        const data = response.data;
    ...
    try {
        const response = await resolveCallAPI(
            'listMedia',
            {"x-session-token":"{{token}}"},
            {"workspaceId":"0","channelId":"{{channelId}}","type":1,"limit":1},
            contextData
        );
        const data = response.data;
    ...
    try {
        const response = await resolveCallAPI(
            'listMedia',
            {"x-session-token":"{{token}}"},
            {"workspaceId":"0","channelId":"{{channelId}}","type":1,"limit":1,"nextPageToken":"invalid_ULID"},
            contextData
        );
        const data = response.data;
      ...
      afterEach(async () => {
          testCaseNumber++;
          const afterEachSteps = ListMediaRequest.options[0]?.afterEach || [];
         ...
         afterAll(async () => {
          const afterAllSteps = ListMediaRequest.options[0]?.afterAll || [];
          ...
          console.log(`📝 Saved result for list-media chunk single to ${filePath}`);
    });
        })
