    ...
    import { ListDmMessageRequest } from '././list-dm-message.request';
    describe('Testcase for list-dm-message', () => {
        let totalTests = 0;
        let allSteps = [];
        let failedTests: any[] = [];
        let codedTest: any[] = [];
        ...
        beforeAll(async () => {
          ...
          const beforeAllSteps = ListDmMessageRequest.options
        ...
        beforeEach(async () => {
          testCaseNumber++;
          const beforeEachSteps = ListDmMessageRequest.options
      ...
      afterEach(async () => {
         ...
         afterAll(async () => {
          const afterAllSteps = ListDmMessageRequest.options
  ...
  const fileName = 'list-dm-message' + (chunkNumber ? `-chunk-undefined` : '') + '.result.json';
  const filePath = path.join(reportDir, fileName);
  fs.writeFileSync(filePath, JSON.stringify(testResult, null, 2), 'utf-8');

  console.log(`📝 Saved result for list-dm-message chunk single to ${filePath}`);
    });
        })
