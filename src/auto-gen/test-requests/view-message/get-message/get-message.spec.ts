    ...
    describe('Testcase for get-message', () => {
        let totalTests = 0;
        let allSteps = [];
        let failedTests: any[] = [];
        let codedTest: any[] = [];
        ...
        beforeAll(async () => {
          ...
          const beforeAllSteps = GetMessageRequest.options
        ...
        beforeEach(async () => {
          testCaseNumber++;
          const beforeEachSteps = GetMessageRequest.options
    ...
    try {
      ...
      afterEach(async () => {
          testCaseNumber++;
          const afterEachSteps = GetMessageRequest.options
         ...
         afterAll(async () => {
          const afterAllSteps = GetMessageRequest.options
  ...
  const fileName = 'get-message' + (chunkNumber ? `-chunk-undefined` : '') + '.result.json';
  const filePath = path.join(reportDir, fileName);
  fs.writeFileSync(filePath, JSON.stringify(testResult, null, 2), 'utf-8');

  console.log(`📝 Saved result for get-message chunk single to ${filePath}`);
    });
        })
