    ...
    import { GetPinnedMessageRequest } from '././get-pinned-message.request';
    describe('Testcase for get-pinned-message', () => {
        let totalTests = 0;
        let allSteps = [];
        let failedTests: any[] = [];
        let codedTest: any[] = [];
        ...
        beforeAll(async () => {
          ...
          const beforeAllSteps = GetPinnedMessageRequest.options
        ...
        beforeEach(async () => {
          testCaseNumber++;
          const beforeEachSteps = GetPinnedMessageRequest.options
      ...
      afterEach(async () => {
          testCaseNumber++;
          const afterEachSteps = GetPinnedMessageRequest.options
         ...
         afterAll(async () => {
          const afterAllSteps = GetPinnedMessageRequest.options
  ...
  const fileName = 'get-pinned-message' + (chunkNumber ? `-chunk-undefined` : '') + '.result.json';
  const filePath = path.join(reportDir, fileName);
  fs.writeFileSync(filePath, JSON.stringify(testResult, null, 2), 'utf-8');

  console.log(`📝 Saved result for get-pinned-message chunk single to ${filePath}`);
    });
        })
