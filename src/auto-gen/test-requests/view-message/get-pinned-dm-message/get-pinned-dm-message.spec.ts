    ...
    import { GetPinnedDmMessageRequest } from '././get-pinned-dm-message.request';
    describe('Testcase for get-pinned-dm-message', () => {
        let totalTests = 0;
        let allSteps = [];
        let failedTests: any[] = [];
        let codedTest: any[] = [];
        ...
        beforeAll(async () => {
          ...
          const beforeAllSteps = GetPinnedDmMessageRequest.options
        ...
        beforeEach(async () => {
          testCaseNumber++;
          const beforeEachSteps = GetPinnedDmMessageRequest.options
      ...
      afterEach(async () => {
          testCaseNumber++;
          const afterEachSteps = GetPinnedDmMessageRequest.options
         ...
         afterAll(async () => {
          const afterAllSteps = GetPinnedDmMessageRequest.options
          ...
          const testResult: TestResult = {
            ...
            className: 'get-pinned-dm-message',
            allSteps: allSteps,
  ...
  const fileName = 'get-pinned-dm-message' + (chunkNumber ? `-chunk-undefined` : '') + '.result.json';
  const filePath = path.join(reportDir, fileName);
  fs.writeFileSync(filePath, JSON.stringify(testResult, null, 2), 'utf-8');

  console.log(`📝 Saved result for get-pinned-dm-message chunk single to ${filePath}`);
    });
        })
