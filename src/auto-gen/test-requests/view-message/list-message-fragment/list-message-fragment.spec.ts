    ...
    import { ListMessageFragmentRequest } from '././list-message-fragment.request';
    describe('Testcase for list-message-fragment', () => {
        let totalTests = 0;
        let allSteps = [];
        let failedTests: any[] = [];
        let codedTest: any[] = [];
        ...
        beforeAll(async () => {
          ...
          const beforeAllSteps = ListMessageFragmentRequest.options
        ...
        beforeEach(async () => {
          testCaseNumber++;
          const beforeEachSteps = ListMessageFragmentRequest.options
    ...
    try {
      ...
      afterEach(async () => {
          testCaseNumber++;
          const afterEachSteps = ListMessageFragmentRequest.options
         ...
         afterAll(async () => {
          const afterAllSteps = ListMessageFragmentRequest.options
          ...
          const testResult: TestResult = {
            ...
            className: 'list-message-fragment',
            allSteps: allSteps,
  ...
  const fileName = 'list-message-fragment' + (chunkNumber ? `-chunk-undefined` : '') + '.result.json';
  const filePath = path.join(reportDir, fileName);
  fs.writeFileSync(filePath, JSON.stringify(testResult, null, 2), 'utf-8');

  console.log(`📝 Saved result for list-message-fragment chunk single to ${filePath}`);
    });
        })
