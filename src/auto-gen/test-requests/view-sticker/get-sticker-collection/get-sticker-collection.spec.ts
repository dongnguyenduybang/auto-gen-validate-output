    ...
    import { GetStickerCollectionRequest } from '././get-sticker-collection.request';
    describe('Testcase for get-sticker-collection', () => {
        let totalTests = 0;
        let allSteps = [];
        let failedTests: any[] = [];
        let codedTest: any[] = [];
        ...
        beforeAll(async () => {
          ...
          const beforeAllSteps = GetStickerCollectionRequest.options
        ...
        beforeEach(async () => {
          testCaseNumber++;
          const beforeEachSteps = GetStickerCollectionRequest.options
    ...
    try {
      ...
      afterEach(async () => {
          testCaseNumber++;
          const afterEachSteps = GetStickerCollectionRequest.options
         ...
         afterAll(async () => {
          const afterAllSteps = GetStickerCollectionRequest.options
          ...
          const testResult: TestResult = {
            path: '/StickerView/GetStickerCollection',
            className: 'get-sticker-collection',
            allSteps: allSteps,
  ...
  const fileName = 'get-sticker-collection' + (chunkNumber ? `-chunk-undefined` : '') + '.result.json';
  const filePath = path.join(reportDir, fileName);
  fs.writeFileSync(filePath, JSON.stringify(testResult, null, 2), 'utf-8');

  console.log(`📝 Saved result for get-sticker-collection chunk single to ${filePath}`);
    });
        })
