    ...
    import { PinDmMessageRequest } from '././pin-dm-message.request';
    describe('Testcase for pin-dm-message', () => {
        let totalTests = 0;
        let allSteps = [];
        let failedTests: any[] = [];
        let codedTest: any[] = [];
        ...
        beforeAll(async () => {
          ...
          const beforeAllSteps = PinDmMessageRequest.options
        ...
        beforeEach(async () => {
          testCaseNumber++;
          const beforeEachSteps = PinDmMessageRequest.options
      ...
      afterEach(async () => {
          testCaseNumber++;
          const afterEachSteps = PinDmMessageRequest.options
         ...
         afterAll(async () => {
          const afterAllSteps = PinDmMessageRequest.options
  ...
  const fileName = 'pin-dm-message' + (chunkNumber ? `-chunk-undefined` : '') + '.result.json';
  const filePath = path.join(reportDir, fileName);
  fs.writeFileSync(filePath, JSON.stringify(testResult, null, 2), 'utf-8');

  console.log(`📝 Saved result for pin-dm-message chunk single to ${filePath}`);
    });
        })
