    ...
    import { MarkDmAsReadRequest } from '././mark-dm-as-read.request';
    describe('Testcase for mark-dm-as-read', () => {
        let totalTests = 0;
        let allSteps = [];
        let failedTests: any[] = [];
        let codedTest: any[] = [];
        ...
        beforeAll(async () => {
          ...
          const beforeAllSteps = MarkDmAsReadRequest.options[0]?.beforeAll || [];
        ...
        beforeEach(async () => {
          testCaseNumber++;
          const beforeEachSteps = MarkDmAsReadRequest.options[0]?.beforeEach || [];
    ...
    try {
      ...
      afterEach(async () => {
          testCaseNumber++;
          const afterEachSteps = MarkDmAsReadRequest.options[0]?.afterEach || [];
         ...
         afterAll(async () => {
          const afterAllSteps = MarkDmAsReadRequest.options[0]?.afterAll || [];
          ...
          const fileName = 'mark-dm-as-read' + (chunkNumber ? `-chunk-undefined` : '') + '.result.json';
          const filePath = path.join(reportDir, fileName);
          fs.writeFileSync(filePath, JSON.stringify(testResult, null, 2), 'utf-8');

          console.log(`📝 Saved result for mark-dm-as-read chunk single to ${filePath}`);
    });
        })
