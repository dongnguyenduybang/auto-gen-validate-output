    ...
    describe('Testcase for add-dm-message-reaction', () => {
        ...
        beforeAll(async () => {
          ...
          const beforeAllSteps = AddDmMessageReactionRequest.options[0]?.beforeAll || [];
        ...
        beforeEach(async () => {
          testCaseNumber++;
          const beforeEachSteps = AddDmMessageReactionRequest.options[0]?.beforeEach || [];
    ...
    try {
      ...
      afterEach(async () => {
          testCaseNumber++;
          const afterEachSteps = AddDmMessageReactionRequest.options[0]?.afterEach || [];
         ...
         afterAll(async () => {
          const afterAllSteps = AddDmMessageReactionRequest.options[0]?.afterAll || [];
          ...
          const fileName = 'add-dm-message-reaction' + (chunkNumber ? `-chunk-undefined` : '') + '.result.json';
          const filePath = path.join(reportDir, fileName);
          fs.writeFileSync(filePath, JSON.stringify(testResult, null, 2), 'utf-8');

          console.log(`📝 Saved result for add-dm-message-reaction chunk single to ${filePath}`);
    });
        })
