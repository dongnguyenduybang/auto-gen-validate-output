    ...
    describe('Testcase for mark-as-read', () => {
        ...
        beforeAll(async () => {
          ...
          const beforeAllSteps = MarkAsReadRequest.options[0]?.beforeAll || [];
        ...
        beforeEach(async () => {
          testCaseNumber++;
          const beforeEachSteps = MarkAsReadRequest.options[0]?.beforeEach || [];
    ...
    try {
      ...
      afterEach(async () => {
          testCaseNumber++;
          const afterEachSteps = MarkAsReadRequest.options[0]?.afterEach || [];
         ...
         afterAll(async () => {
          const afterAllSteps = MarkAsReadRequest.options[0]?.afterAll || [];
          ...
          const fileName = 'mark-as-read' + (chunkNumber ? `-chunk-undefined` : '') + '.result.json';
          const filePath = path.join(reportDir, fileName);
          fs.writeFileSync(filePath, JSON.stringify(testResult, null, 2), 'utf-8');

          console.log(`📝 Saved result for mark-as-read chunk single to ${filePath}`);
    });
        })
