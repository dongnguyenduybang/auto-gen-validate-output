...
describe('Testcase for send-message', () => {
  ...
  beforeAll(async () => {
    ...
    const beforeAllSteps =
      SendMessageRequest.options?.find((option) => option.beforeAll)
        ?.beforeAll || [];
  ...
  beforeEach(async () => {
    ...
    const beforeEachSteps =
      SendMessageRequest.options?.find((option) => option.beforeEach)
        ?.beforeEach || [];
  ...
  afterEach(async () => {
    ...
    const afterEachSteps =
      SendMessageRequest.options?.find((option) => option.afterEach)
        ?.afterEach || [];
  ...
  afterAll(async () => {
    const afterAllSteps =
      SendMessageRequest.options?.find((option) => option.afterAll)?.afterAll ||
      [];
    ...
    const fileName =
      'send-message' + (chunkNumber ? `-chunk-undefined` : '') + '.result.json';
