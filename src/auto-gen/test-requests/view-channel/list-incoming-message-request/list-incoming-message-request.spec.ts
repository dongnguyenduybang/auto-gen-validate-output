    ...
    import { ListIncomingMessageRequestRequest } from '././list-incoming-message-request.request';
    describe('Testcase for list-incoming-message-request', () => {
        let totalTests = 0;
        let allSteps = [];
        let failedTests: any[] = [];
        let codedTest: any[] = [];
        ...
        beforeAll(async () => {
          ...
          const beforeAllSteps = ListIncomingMessageRequestRequest.options
        ...
        beforeEach(async () => {
          testCaseNumber++;
          const beforeEachSteps = ListIncomingMessageRequestRequest.options
      ...
      afterEach(async () => {
          testCaseNumber++;
          const afterEachSteps = ListIncomingMessageRequestRequest.options
         ...
         afterAll(async () => {
          const afterAllSteps = ListIncomingMessageRequestRequest.options
          ...
          const testResult: TestResult = {
            ...
            className: 'list-incoming-message-request',
            allSteps: allSteps,
  ...
  const fileName = 'list-incoming-message-request' + (chunkNumber ? `-chunk-undefined` : '') + '.result.json';
  const filePath = path.join(reportDir, fileName);
  fs.writeFileSync(filePath, JSON.stringify(testResult, null, 2), 'utf-8');

  console.log(`📝 Saved result for list-incoming-message-request chunk single to ${filePath}`);
    });
        })
