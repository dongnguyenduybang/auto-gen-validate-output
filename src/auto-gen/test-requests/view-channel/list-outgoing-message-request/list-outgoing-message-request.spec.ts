    ...
    import { ListOutgoingMessageRequestRequest } from '././list-outgoing-message-request.request';
    describe('Testcase for list-outgoing-message-request', () => {
        let totalTests = 0;
        let allSteps = [];
        let failedTests: any[] = [];
        let codedTest: any[] = [];
        ...
        beforeAll(async () => {
          ...
          const beforeAllSteps = ListOutgoingMessageRequestRequest.options
        ...
        beforeEach(async () => {
          testCaseNumber++;
          const beforeEachSteps = ListOutgoingMessageRequestRequest.options
      ...
      afterEach(async () => {
          testCaseNumber++;
          const afterEachSteps = ListOutgoingMessageRequestRequest.options
         ...
         afterAll(async () => {
          const afterAllSteps = ListOutgoingMessageRequestRequest.options
          ...
          const testResult: TestResult = {
            ...
            className: 'list-outgoing-message-request',
            allSteps: allSteps,
  ...
  const fileName = 'list-outgoing-message-request' + (chunkNumber ? `-chunk-undefined` : '') + '.result.json';
  const filePath = path.join(reportDir, fileName);
  fs.writeFileSync(filePath, JSON.stringify(testResult, null, 2), 'utf-8');

  console.log(`📝 Saved result for list-outgoing-message-request chunk single to ${filePath}`);
    });
        })
