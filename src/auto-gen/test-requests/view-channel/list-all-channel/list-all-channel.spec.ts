    ...
    import { ListAllChannelRequest } from '././list-all-channel.request';
    describe('Testcase for list-all-channel', () => {
        let totalTests = 0;
        let allSteps = [];
        let failedTests: any[] = [];
        let codedTest: any[] = [];
        ...
        beforeAll(async () => {
          ...
          const beforeAllSteps = ListAllChannelRequest.options
        ...
        beforeEach(async () => {
          testCaseNumber++;
          const beforeEachSteps = ListAllChannelRequest.options
    ...
    try {
      ...
      afterEach(async () => {
          testCaseNumber++;
          const afterEachSteps = ListAllChannelRequest.options
         ...
         afterAll(async () => {
          const afterAllSteps = ListAllChannelRequest.options
  ...
  const fileName = 'list-all-channel' + (chunkNumber ? `-chunk-undefined` : '') + '.result.json';
  const filePath = path.join(reportDir, fileName);
  fs.writeFileSync(filePath, JSON.stringify(testResult, null, 2), 'utf-8');

  console.log(`📝 Saved result for list-all-channel chunk single to ${filePath}`);
    });
        })
