    ...
    import { ListChannelRequest } from '././list-channel.request';
    describe('Testcase for list-channel', () => {
        let totalTests = 0;
        let allSteps = [];
        let failedTests: any[] = [];
        let codedTest: any[] = [];
        ...
        beforeAll(async () => {
          ...
          const beforeAllSteps = ListChannelRequest.options
        ...
        beforeEach(async () => {
          testCaseNumber++;
          const beforeEachSteps = ListChannelRequest.options
      ...
      afterEach(async () => {
          testCaseNumber++;
          const afterEachSteps = ListChannelRequest.options
         ...
         afterAll(async () => {
          const afterAllSteps = ListChannelRequest.options
  ...
  const fileName = 'list-channel' + (chunkNumber ? `-chunk-undefined` : '') + '.result.json';
  const filePath = path.join(reportDir, fileName);
  fs.writeFileSync(filePath, JSON.stringify(testResult, null, 2), 'utf-8');

  console.log(`📝 Saved result for list-channel chunk single to ${filePath}`);
    });
        })
