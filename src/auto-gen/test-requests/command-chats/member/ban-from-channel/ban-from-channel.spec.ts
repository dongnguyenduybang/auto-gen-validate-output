    ...
    import { BanFromChannelRequest } from '././ban-from-channel.request';
    describe('Testcase for ban-from-channel', () => {
        let totalTests = 0;
        let allSteps = [];
        let failedTests: any[] = [];
        let codedTest: any[] = [];
        ...
        beforeAll(async () => {
          ...
          const beforeAllSteps = BanFromChannelRequest.options[0]?.beforeAll || [];
        ...
        beforeEach(async () => {
          testCaseNumber++;
          const beforeEachSteps = BanFromChannelRequest.options[0]?.beforeEach || [];
    ...
    try {
      ...
      afterEach(async () => {
          testCaseNumber++;
          const afterEachSteps = BanFromChannelRequest.options[0]?.afterEach || [];
         ...
         afterAll(async () => {
          const afterAllSteps = BanFromChannelRequest.options[0]?.afterAll || [];
          ...
          const fileName = 'ban-from-channel' + (chunkNumber ? `-chunk-undefined` : '') + '.result.json';
          const filePath = path.join(reportDir, fileName);
          fs.writeFileSync(filePath, JSON.stringify(testResult, null, 2), 'utf-8');

          console.log(`📝 Saved result for ban-from-channel chunk single to ${filePath}`);
    });
        })
