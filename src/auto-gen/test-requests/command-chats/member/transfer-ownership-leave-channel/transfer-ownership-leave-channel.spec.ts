    ...
    import { TransferOwnershipLeaveChannelRequest } from '././transfer-ownership-leave-channel.request';
    describe('Testcase for transfer-ownership-leave-channel', () => {
        let totalTests = 0;
        let allSteps = [];
        let failedTests: any[] = [];
        let codedTest: any[] = [];
        ...
        beforeAll(async () => {
          ...
          const beforeAllSteps = TransferOwnershipLeaveChannelRequest.options[0]?.beforeAll || [];
        ...
        beforeEach(async () => {
          testCaseNumber++;
          const beforeEachSteps = TransferOwnershipLeaveChannelRequest.options[0]?.beforeEach || [];
    ...
    try {
      ...
      afterEach(async () => {
          testCaseNumber++;
          const afterEachSteps = TransferOwnershipLeaveChannelRequest.options[0]?.afterEach || [];
         ...
         afterAll(async () => {
          const afterAllSteps = TransferOwnershipLeaveChannelRequest.options[0]?.afterAll || [];
          ...
          const testResult: TestResult = {
            ...
            className: 'transfer-ownership-leave-channel',
            allSteps: allSteps,
          ...
          const fileName = 'transfer-ownership-leave-channel' + (chunkNumber ? `-chunk-undefined` : '') + '.result.json';
          const filePath = path.join(reportDir, fileName);
          fs.writeFileSync(filePath, JSON.stringify(testResult, null, 2), 'utf-8');

          console.log(`📝 Saved result for transfer-ownership-leave-channel chunk single to ${filePath}`);
    });
        })
