    ...
    describe('Testcase for transfer-ownership', () => {
        let totalTests = 0;
        let allSteps = [];
        let failedTests: any[] = [];
        let codedTest: any[] = [];
        ...
        beforeAll(async () => {
          ...
          const beforeAllSteps = TransferOwnershipRequest.options[0]?.beforeAll || [];
        ...
        beforeEach(async () => {
          testCaseNumber++;
          const beforeEachSteps = TransferOwnershipRequest.options[0]?.beforeEach || [];
    ...
    try {
      ...
      afterEach(async () => {
          testCaseNumber++;
          const afterEachSteps = TransferOwnershipRequest.options[0]?.afterEach || [];
         ...
         afterAll(async () => {
          const afterAllSteps = TransferOwnershipRequest.options[0]?.afterAll || [];
          ...
          const fileName = 'transfer-ownership' + (chunkNumber ? `-chunk-undefined` : '') + '.result.json';
          const filePath = path.join(reportDir, fileName);
          fs.writeFileSync(filePath, JSON.stringify(testResult, null, 2), 'utf-8');

          console.log(`📝 Saved result for transfer-ownership chunk single to ${filePath}`);
    });
        })
