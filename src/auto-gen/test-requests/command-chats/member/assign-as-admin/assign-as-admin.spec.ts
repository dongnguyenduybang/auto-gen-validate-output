    ...
    import { AssignAsAdminRequest } from '././assign-as-admin.request';
    describe('Testcase for assign-as-admin', () => {
        let totalTests = 0;
        let allSteps = [];
        let failedTests: any[] = [];
        let codedTest: any[] = [];
        ...
        beforeAll(async () => {
          ...
          const beforeAllSteps = AssignAsAdminRequest.options[0]?.beforeAll || [];
        ...
        beforeEach(async () => {
          testCaseNumber++;
          const beforeEachSteps = AssignAsAdminRequest.options[0]?.beforeEach || [];
    ...
    try {
        ...
        failedTests.push({
            ...
            it('Test case #45 should return errors ["Unsupported permission type"] when body {"workspaceId":"invalid_value","channelId":"invalid_value","userId":"{{userId1}}"}', async () => {
    testNumber = 45;
    totalTests++;
    ...
    try {
      ...
      afterEach(async () => {
          testCaseNumber++;
          const afterEachSteps = AssignAsAdminRequest.options[0]?.afterEach || [];
         ...
         afterAll(async () => {
          const afterAllSteps = AssignAsAdminRequest.options[0]?.afterAll || [];
          ...
          const fileName = 'assign-as-admin' + (chunkNumber ? `-chunk-undefined` : '') + '.result.json';
          const filePath = path.join(reportDir, fileName);
          fs.writeFileSync(filePath, JSON.stringify(testResult, null, 2), 'utf-8');

          console.log(`📝 Saved result for assign-as-admin chunk single to ${filePath}`);
    });
        })
