    ...
    import { UnbanFromChannelRequest } from '././unban-from-channel.request';
    describe('Testcase for unban-from-channel', () => {
        let totalTests = 0;
        let allSteps = [];
        let failedTests: any[] = [];
        let codedTest: any[] = [];
        ...
        beforeAll(async () => {
          ...
          const beforeAllSteps = UnbanFromChannelRequest.options[0]?.beforeAll || [];
        ...
        beforeEach(async () => {
          testCaseNumber++;
          const beforeEachSteps = UnbanFromChannelRequest.options[0]?.beforeEach || [];
    ...
    try {
      ...
      afterEach(async () => {
          testCaseNumber++;
          const afterEachSteps = UnbanFromChannelRequest.options[0]?.afterEach || [];
         ...
         afterAll(async () => {
          const afterAllSteps = UnbanFromChannelRequest.options[0]?.afterAll || [];
          ...
          const fileName = 'unban-from-channel' + (chunkNumber ? `-chunk-undefined` : '') + '.result.json';
          const filePath = path.join(reportDir, fileName);
          fs.writeFileSync(filePath, JSON.stringify(testResult, null, 2), 'utf-8');

          console.log(`📝 Saved result for unban-from-channel chunk single to ${filePath}`);
    });
        })
