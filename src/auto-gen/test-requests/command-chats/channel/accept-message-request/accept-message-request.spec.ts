    ...
    import { AcceptMessageRequestRequest } from '././accept-message-request.request';
    describe('Testcase for accept-message-request', () => {
        let totalTests = 0;
        let allSteps = [];
        let failedTests: any[] = [];
        let codedTest: any[] = [];
        ...
        beforeAll(async () => {
          ...
          const beforeAllSteps = AcceptMessageRequestRequest.options[0]?.beforeAll || [];
        ...
        beforeEach(async () => {
          testCaseNumber++;
          const beforeEachSteps = AcceptMessageRequestRequest.options[0]?.beforeEach || [];
    ...
    try {
      ...
      afterEach(async () => {
          testCaseNumber++;
          const afterEachSteps = AcceptMessageRequestRequest.options[0]?.afterEach || [];
         ...
         afterAll(async () => {
          const afterAllSteps = AcceptMessageRequestRequest.options[0]?.afterAll || [];
          ...
          const testResult: TestResult = {
            ...
            className: 'accept-message-request',
            allSteps: allSteps,
          ...
          const fileName = 'accept-message-request' + (chunkNumber ? `-chunk-undefined` : '') + '.result.json';
          const filePath = path.join(reportDir, fileName);
          fs.writeFileSync(filePath, JSON.stringify(testResult, null, 2), 'utf-8');

          console.log(`📝 Saved result for accept-message-request chunk single to ${filePath}`);
    });
        })
