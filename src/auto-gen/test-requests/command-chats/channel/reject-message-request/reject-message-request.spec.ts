    ...
    import { RejectMessageRequestRequest } from '././reject-message-request.request';
    describe('Testcase for reject-message-request', () => {
        let totalTests = 0;
        let allSteps = [];
        let failedTests: any[] = [];
        let codedTest: any[] = [];
        ...
        beforeAll(async () => {
          ...
          const beforeAllSteps = RejectMessageRequestRequest.options[0]?.beforeAll || [];
        ...
        beforeEach(async () => {
          testCaseNumber++;
          const beforeEachSteps = RejectMessageRequestRequest.options[0]?.beforeEach || [];
    ...
    try {
      ...
      afterEach(async () => {
          testCaseNumber++;
          const afterEachSteps = RejectMessageRequestRequest.options[0]?.afterEach || [];
         ...
         afterAll(async () => {
          const afterAllSteps = RejectMessageRequestRequest.options[0]?.afterAll || [];
          ...
          const testResult: TestResult = {
            ...
            className: 'reject-message-request',
            allSteps: allSteps,
          ...
          const fileName = 'reject-message-request' + (chunkNumber ? `-chunk-undefined` : '') + '.result.json';
          const filePath = path.join(reportDir, fileName);
          fs.writeFileSync(filePath, JSON.stringify(testResult, null, 2), 'utf-8');

          console.log(`📝 Saved result for reject-message-request chunk single to ${filePath}`);
    });
        })
