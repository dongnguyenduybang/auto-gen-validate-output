    ...
    import { DeleteChannelAvatarRequest } from '././delete-channel-avatar.request';
    describe('Testcase for delete-channel-avatar', () => {
        let totalTests = 0;
        let allSteps = [];
        let failedTests: any[] = [];
        let codedTest: any[] = [];
        ...
        beforeAll(async () => {
          ...
          const beforeAllSteps = DeleteChannelAvatarRequest.options[0]?.beforeAll || [];
        ...
        beforeEach(async () => {
          testCaseNumber++;
          const beforeEachSteps = DeleteChannelAvatarRequest.options[0]?.beforeEach || [];
    ...
    try {
      ...
      afterEach(async () => {
          testCaseNumber++;
          const afterEachSteps = DeleteChannelAvatarRequest.options[0]?.afterEach || [];
         ...
         afterAll(async () => {
          const afterAllSteps = DeleteChannelAvatarRequest.options[0]?.afterAll || [];
          ...
          const testResult: TestResult = {
            ...
            className: 'delete-channel-avatar',
            allSteps: allSteps,
          ...
          const fileName = 'delete-channel-avatar' + (chunkNumber ? `-chunk-undefined` : '') + '.result.json';
          const filePath = path.join(reportDir, fileName);
          fs.writeFileSync(filePath, JSON.stringify(testResult, null, 2), 'utf-8');

          console.log(`📝 Saved result for delete-channel-avatar chunk single to ${filePath}`);
    });
        })
