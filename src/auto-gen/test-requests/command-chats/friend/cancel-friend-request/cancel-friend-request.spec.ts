    ...
    import { CancelFriendRequestRequest } from '././cancel-friend-request.request';
    describe('Testcase for cancel-friend-request', () => {
        let totalTests = 0;
        let allSteps = [];
        let failedTests: any[] = [];
        let codedTest: any[] = [];
        ...
        beforeAll(async () => {
          ...
          const beforeAllSteps = CancelFriendRequestRequest.options[0]?.beforeAll || [];
        ...
        beforeEach(async () => {
          testCaseNumber++;
          const beforeEachSteps = CancelFriendRequestRequest.options[0]?.beforeEach || [];
    ...
    try {
      ...
      afterEach(async () => {
          testCaseNumber++;
          const afterEachSteps = CancelFriendRequestRequest.options[0]?.afterEach || [];
         ...
         afterAll(async () => {
          const afterAllSteps = CancelFriendRequestRequest.options[0]?.afterAll || [];
          ...
          const fileName = 'cancel-friend-request' + (chunkNumber ? `-chunk-undefined` : '') + '.result.json';
          const filePath = path.join(reportDir, fileName);
          fs.writeFileSync(filePath, JSON.stringify(testResult, null, 2), 'utf-8');

          console.log(`📝 Saved result for cancel-friend-request chunk single to ${filePath}`);
    });
        })
