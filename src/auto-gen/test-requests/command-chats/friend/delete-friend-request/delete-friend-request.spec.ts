    ...
    import { DeleteFriendRequestRequest } from '././delete-friend-request.request';
    describe('Testcase for delete-friend-request', () => {
        let totalTests = 0;
        let allSteps = [];
        let failedTests: any[] = [];
        let codedTest: any[] = [];
        ...
        beforeAll(async () => {
          ...
          const beforeAllSteps = DeleteFriendRequestRequest.options[0]?.beforeAll || [];
        ...
        beforeEach(async () => {
          testCaseNumber++;
          const beforeEachSteps = DeleteFriendRequestRequest.options[0]?.beforeEach || [];
    ...
    try {
      ...
      afterEach(async () => {
          testCaseNumber++;
          const afterEachSteps = DeleteFriendRequestRequest.options[0]?.afterEach || [];
         ...
         afterAll(async () => {
          const afterAllSteps = DeleteFriendRequestRequest.options[0]?.afterAll || [];
          ...
          const testResult: TestResult = {
            ...
            className: 'delete-friend-request',
            allSteps: allSteps,
          ...
          const fileName = 'delete-friend-request' + (chunkNumber ? `-chunk-undefined` : '') + '.result.json';
          const filePath = path.join(reportDir, fileName);
          fs.writeFileSync(filePath, JSON.stringify(testResult, null, 2), 'utf-8');

          console.log(`📝 Saved result for delete-friend-request chunk single to ${filePath}`);
    });
        })
