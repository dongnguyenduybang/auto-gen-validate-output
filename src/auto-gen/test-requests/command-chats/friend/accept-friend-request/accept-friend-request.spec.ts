    ...
    import { AcceptFriendRequestRequest } from '././accept-friend-request.request';
    describe('Testcase for accept-friend-request', () => {
        let totalTests = 0;
        let allSteps = [];
        let failedTests: any[] = [];
        let codedTest: any[] = [];
        ...
        beforeAll(async () => {
          ...
          const beforeAllSteps = AcceptFriendRequestRequest.options
        ...
        beforeEach(async () => {
          testCaseNumber++;
          const beforeEachSteps = AcceptFriendRequestRequest.options
      ...
      afterEach(async () => {
          testCaseNumber++;
          const afterEachSteps = AcceptFriendRequestRequest.options
         ...
         afterAll(async () => {
          const afterAllSteps = AcceptFriendRequestRequest.options
  ...
  const fileName = 'accept-friend-request' + (chunkNumber ? `-chunk-undefined` : '') + '.result.json';
  const filePath = path.join(reportDir, fileName);
  fs.writeFileSync(filePath, JSON.stringify(testResult, null, 2), 'utf-8');

  console.log(`📝 Saved result for accept-friend-request chunk single to ${filePath}`);
    });
        })
