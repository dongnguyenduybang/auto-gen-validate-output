    ...
    import { AddFriendRequest } from '././add-friend.request';
    describe('Testcase for add-friend', () => {
        let totalTests = 0;
        let allSteps = [];
        let failedTests: any[] = [];
        let codedTest: any[] = [];
        ...
        beforeAll(async () => {
          ...
          const beforeAllSteps = AddFriendRequest.options[0]?.beforeAll || [];
        ...
        beforeEach(async () => {
          testCaseNumber++;
          const beforeEachSteps = AddFriendRequest.options[0]?.beforeEach || [];
    ...
    try {
      ...
      afterEach(async () => {
          testCaseNumber++;
          const afterEachSteps = AddFriendRequest.options[0]?.afterEach || [];
         ...
         afterAll(async () => {
          const afterAllSteps = AddFriendRequest.options[0]?.afterAll || [];
          ...
          const fileName = 'add-friend' + (chunkNumber ? `-chunk-undefined` : '') + '.result.json';
          const filePath = path.join(reportDir, fileName);
          fs.writeFileSync(filePath, JSON.stringify(testResult, null, 2), 'utf-8');

          console.log(`📝 Saved result for add-friend chunk single to ${filePath}`);
    });
        })
