    ...
    import { UnfriendRequest } from '././unfriend.request';
    describe('Testcase for unfriend', () => {
        let totalTests = 0;
        let allSteps = [];
        let failedTests: any[] = [];
        let codedTest: any[] = [];
        ...
        beforeAll(async () => {
          ...
          const beforeAllSteps = UnfriendRequest.options[0]?.beforeAll || [];
        ...
        beforeEach(async () => {
          testCaseNumber++;
          const beforeEachSteps = UnfriendRequest.options[0]?.beforeEach || [];
    ...
    try {
      ...
      afterEach(async () => {
          testCaseNumber++;
          const afterEachSteps = UnfriendRequest.options[0]?.afterEach || [];
         ...
         afterAll(async () => {
          const afterAllSteps = UnfriendRequest.options[0]?.afterAll || [];
          ...
          const fileName = 'unfriend' + (chunkNumber ? `-chunk-undefined` : '') + '.result.json';
          const filePath = path.join(reportDir, fileName);
          fs.writeFileSync(filePath, JSON.stringify(testResult, null, 2), 'utf-8');

          console.log(`📝 Saved result for unfriend chunk single to ${filePath}`);
    });
        })
