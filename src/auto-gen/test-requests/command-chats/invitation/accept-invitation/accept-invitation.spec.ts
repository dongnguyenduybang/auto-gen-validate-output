    ...
    describe('Testcase for accept-invitation', () => {
        let totalTests = 0;
        let allSteps = [];
        let failedTests: any[] = [];
        let codedTest: any[] = [];
        ...
        beforeAll(async () => {
          ...
          const beforeAllSteps = AcceptInvitationRequest.options[0]?.beforeAll || [];
        ...
        beforeEach(async () => {
          testCaseNumber++;
          const beforeEachSteps = AcceptInvitationRequest.options[0]?.beforeEach || [];
          ...
          if (beforeEachSteps.length > 0) {
            ...
            it('Test case #1 should return errors ["invitationLink expected string,received number"] when body {"invitationLink":123}', async () => {
    testNumber = 1;
    totalTests++;
    const payloadObj = {"invitationLink":123};
    ...
    try {
        ...
        failedTests.push({
            ...
            it('Test case #2 should return errors [] when body {"invitationLink":"{{invitationLink}}"}', async () => {
    testNumber = 2;
    totalTests++;
    const payloadObj = {"invitationLink":"{{invitationLink}}"};
    resolvedData = resolveVariables(payloadObj, globalContext);
    ...
    try {
        ...
        failedTests.push({
            ...
            it('Test case #3 should return errors ["invitationLink required","invitationLink invalid url"] when body {}', async () => {
    testNumber = 3;
    totalTests++;
    const payloadObj = {};
    resolvedData = resolveVariables(payloadObj, globalContext);
    ...
    try {
        ...
        failedTests.push({
            ...
            it('Test case #4 should return errors ["invitationLink should not be empty","invitationLink invalid url"] when body {"invitationLink":""}', async () => {
    testNumber = 4;
    totalTests++;
    const payloadObj = {"invitationLink":""};
    resolvedData = resolveVariables(payloadObj, globalContext);
    ...
    try {
        ...
        failedTests.push({
            ...
            it('Test case #5 should return errors ["invitationLink expected string,received null"] when body {"invitationLink":null}', async () => {
    testNumber = 5;
    totalTests++;
    const payloadObj = {"invitationLink":null};
    resolvedData = resolveVariables(payloadObj, globalContext);
    ...
    try {
      ...
      afterEach(async () => {
          testCaseNumber++;
          const afterEachSteps = AcceptInvitationRequest.options[0]?.afterEach || [];
         ...
         afterAll(async () => {
          const afterAllSteps = AcceptInvitationRequest.options[0]?.afterAll || [];
          ...
          const fileName = 'accept-invitation' + (chunkNumber ? `-chunk-undefined` : '') + '.result.json';
          const filePath = path.join(reportDir, fileName);
          fs.writeFileSync(filePath, JSON.stringify(testResult, null, 2), 'utf-8');

          console.log(`📝 Saved result for accept-invitation chunk single to ${filePath}`);
    });
        })
