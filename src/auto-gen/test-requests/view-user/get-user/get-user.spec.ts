    ...
    describe('Testcase for get-user', () => {
        let totalTests = 0;
        let allSteps = [];
        let failedTests: any[] = [];
        let codedTest: any[] = [];
        ...
        beforeAll(async () => {
          ...
          const beforeAllSteps = GetUserRequest.options
        ...
        beforeEach(async () => {
          testCaseNumber++;
          const beforeEachSteps = GetUserRequest.options
      ...
      afterEach(async () => {
          testCaseNumber++;
          const afterEachSteps = GetUserRequest.options
         ...
         afterAll(async () => {
          const afterAllSteps = GetUserRequest.options
  ...
  const fileName = 'get-user' + (chunkNumber ? `-chunk-undefined` : '') + '.result.json';
  const filePath = path.join(reportDir, fileName);
  fs.writeFileSync(filePath, JSON.stringify(testResult, null, 2), 'utf-8');

  console.log(`📝 Saved result for get-user chunk single to ${filePath}`);
    });
        })
