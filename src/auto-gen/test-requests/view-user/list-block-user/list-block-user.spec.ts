    ...
    import { ListBlockUserRequest } from '././list-block-user.request';
    describe('Testcase for list-block-user', () => {
        let totalTests = 0;
        let allSteps = [];
        let failedTests: any[] = [];
        let codedTest: any[] = [];
        ...
        beforeAll(async () => {
          ...
          const beforeAllSteps = ListBlockUserRequest.options
        ...
        beforeEach(async () => {
          testCaseNumber++;
          const beforeEachSteps = ListBlockUserRequest.options
    ...
    try {
      ...
      afterEach(async () => {
          testCaseNumber++;
          const afterEachSteps = ListBlockUserRequest.options
         ...
         afterAll(async () => {
          const afterAllSteps = ListBlockUserRequest.options
  ...
  const fileName = 'list-block-user' + (chunkNumber ? `-chunk-undefined` : '') + '.result.json';
  const filePath = path.join(reportDir, fileName);
  fs.writeFileSync(filePath, JSON.stringify(testResult, null, 2), 'utf-8');

  console.log(`📝 Saved result for list-block-user chunk single to ${filePath}`);
    });
        })
