    ...
    import { BlockUserRequest } from '././block-user.request';
    describe('Testcase for block-user', () => {
        let totalTests = 0;
        let allSteps = [];
        let failedTests: any[] = [];
        let codedTest: any[] = [];
        ...
        beforeAll(async () => {
          ...
          const beforeAllSteps = BlockUserRequest.options
        ...
        beforeEach(async () => {
          testCaseNumber++;
          const beforeEachSteps = BlockUserRequest.options
    ...
    try {
      ...
      afterEach(async () => {
          testCaseNumber++;
          const afterEachSteps = BlockUserRequest.options
         ...
         afterAll(async () => {
          const afterAllSteps = BlockUserRequest.options
  ...
  const fileName = 'block-user' + (chunkNumber ? `-chunk-undefined` : '') + '.result.json';
  const filePath = path.join(reportDir, fileName);
  fs.writeFileSync(filePath, JSON.stringify(testResult, null, 2), 'utf-8');

  console.log(`📝 Saved result for block-user chunk single to ${filePath}`);
    });
        })
