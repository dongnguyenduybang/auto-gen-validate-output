    ...
    import { UnblockUserRequest } from '././unblock-user.request';
    describe('Testcase for unblock-user', () => {
        let totalTests = 0;
        let allSteps = [];
        let failedTests: any[] = [];
        let codedTest: any[] = [];
        ...
        beforeAll(async () => {
          ...
          const beforeAllSteps = UnblockUserRequest.options
        ...
        beforeEach(async () => {
          testCaseNumber++;
          const beforeEachSteps = UnblockUserRequest.options
          ...
          if (beforeEachSteps.length > 0) {
            ...
            it('Test case #1 should return errors ["Could not resolve permission type"] when body {"targetUserId":123}', async () => {
              testNumber = 1;
              totalTests++;
              const payloadObj = {"targetUserId":123};
            ...
            it('Test case #2 should return errors [] when body {"targetUserId":"{{userId1}}"}', async () => {
              testNumber = 2;
              totalTests++;
              const payloadObj = {"targetUserId":"{{userId1}}"};
              resolvedData = resolveVariables(payloadObj, globalContext);
            ...
            it('Test case #4 should return errors ["Could not resolve permission type"] when body {"targetUserId":""}', async () => {
              testNumber = 4;
              totalTests++;
              const payloadObj = {"targetUserId":""};
              resolvedData = resolveVariables(payloadObj, globalContext);
            ...
            it('Test case #5 should return errors ["Could not resolve permission type"] when body {"targetUserId":null}', async () => {
              testNumber = 5;
              totalTests++;
              const payloadObj = {"targetUserId":null};
              resolvedData = resolveVariables(payloadObj, globalContext);
            ...
            it('Test case #6 should return errors [] when body {"targetUserId":"invalid_value"}', async () => {
              testNumber = 6;
              totalTests++;
              const payloadObj = {"targetUserId":"invalid_value"};
              resolvedData = resolveVariables(payloadObj, globalContext);
      ...
      afterEach(async () => {
          testCaseNumber++;
          const afterEachSteps = UnblockUserRequest.options
         ...
         afterAll(async () => {
          const afterAllSteps = UnblockUserRequest.options
  ...
  const fileName = 'unblock-user' + (chunkNumber ? `-chunk-undefined` : '') + '.result.json';
  const filePath = path.join(reportDir, fileName);
  fs.writeFileSync(filePath, JSON.stringify(testResult, null, 2), 'utf-8');

  console.log(`📝 Saved result for unblock-user chunk single to ${filePath}`);
    });
        })
