    ...
    import { AddCoverPhotoRequest } from '././add-cover-photo.request';
    describe('Testcase for add-cover-photo', () => {
        let totalTests = 0;
        let allSteps = [];
        let failedTests: any[] = [];
        let codedTest: any[] = [];
        ...
        beforeAll(async () => {
          ...
          const beforeAllSteps = AddCoverPhotoRequest.options
        ...
        beforeEach(async () => {
          testCaseNumber++;
          const beforeEachSteps = AddCoverPhotoRequest.options
    ...
    try {
      ...
      afterEach(async () => {
          testCaseNumber++;
          const afterEachSteps = AddCoverPhotoRequest.options
         ...
         afterAll(async () => {
          const afterAllSteps = AddCoverPhotoRequest.options
  ...
  const fileName = 'add-cover-photo' + (chunkNumber ? `-chunk-undefined` : '') + '.result.json';
  const filePath = path.join(reportDir, fileName);
  fs.writeFileSync(filePath, JSON.stringify(testResult, null, 2), 'utf-8');

  console.log(`📝 Saved result for add-cover-photo chunk single to ${filePath}`);
    });
        })
