    ...
    import { DeleteUserVisitProfileRequest } from '././delete-user-visit-profile.request';
    describe('Testcase for delete-user-visit-profile', () => {
        let totalTests = 0;
        let allSteps = [];
        let failedTests: any[] = [];
        let codedTest: any[] = [];
        ...
        beforeAll(async () => {
          ...
          const beforeAllSteps = DeleteUserVisitProfileRequest.options
        ...
        beforeEach(async () => {
          testCaseNumber++;
          const beforeEachSteps = DeleteUserVisitProfileRequest.options
      ...
      afterEach(async () => {
          testCaseNumber++;
          const afterEachSteps = DeleteUserVisitProfileRequest.options
         ...
         afterAll(async () => {
          const afterAllSteps = DeleteUserVisitProfileRequest.options
          ...
          const testResult: TestResult = {
            ...
            className: 'delete-user-visit-profile',
            allSteps: allSteps,
  ...
  const fileName = 'delete-user-visit-profile' + (chunkNumber ? `-chunk-undefined` : '') + '.result.json';
  const filePath = path.join(reportDir, fileName);
  fs.writeFileSync(filePath, JSON.stringify(testResult, null, 2), 'utf-8');

  console.log(`📝 Saved result for delete-user-visit-profile chunk single to ${filePath}`);
    });
        })
