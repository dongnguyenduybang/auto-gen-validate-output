    ...
    import { VisitProfileRequest } from '././visit-profile.request';
    describe('Testcase for visit-profile', () => {
        let totalTests = 0;
        let allSteps = [];
        let failedTests: any[] = [];
        let codedTest: any[] = [];
        ...
        beforeAll(async () => {
          ...
          const beforeAllSteps = VisitProfileRequest.options
        ...
        beforeEach(async () => {
          testCaseNumber++;
          const beforeEachSteps = VisitProfileRequest.options
    ...
    try {
      ...
      afterEach(async () => {
          testCaseNumber++;
          const afterEachSteps = VisitProfileRequest.options
         ...
         afterAll(async () => {
          const afterAllSteps = VisitProfileRequest.options
  ...
  const fileName = 'visit-profile' + (chunkNumber ? `-chunk-undefined` : '') + '.result.json';
  const filePath = path.join(reportDir, fileName);
  fs.writeFileSync(filePath, JSON.stringify(testResult, null, 2), 'utf-8');

  console.log(`📝 Saved result for visit-profile chunk single to ${filePath}`);
    });
        })
