    ...
    import { UpdateUserAvatarRequest } from '././update-user-avatar.request';
    describe('Testcase for update-user-avatar', () => {
        let totalTests = 0;
        let allSteps = [];
        let failedTests: any[] = [];
        let codedTest: any[] = [];
        ...
        beforeAll(async () => {
          ...
          const beforeAllSteps = UpdateUserAvatarRequest.options
        ...
        beforeEach(async () => {
          testCaseNumber++;
          const beforeEachSteps = UpdateUserAvatarRequest.options
    ...
    try {
      ...
      afterEach(async () => {
          testCaseNumber++;
          const afterEachSteps = UpdateUserAvatarRequest.options
         ...
         afterAll(async () => {
          const afterAllSteps = UpdateUserAvatarRequest.options
  ...
  const fileName = 'update-user-avatar' + (chunkNumber ? `-chunk-undefined` : '') + '.result.json';
  const filePath = path.join(reportDir, fileName);
  fs.writeFileSync(filePath, JSON.stringify(testResult, null, 2), 'utf-8');

  console.log(`📝 Saved result for update-user-avatar chunk single to ${filePath}`);
    });
        })
