    ...
    describe('Testcase for update-user-email', () => {
        let totalTests = 0;
        let allSteps = [];
        let failedTests: any[] = [];
        let codedTest: any[] = [];
        ...
        beforeAll(async () => {
          ...
          const beforeAllSteps = UpdateUserEmailRequest.options
        ...
        beforeEach(async () => {
          testCaseNumber++;
          const beforeEachSteps = UpdateUserEmailRequest.options
          ...
          if (beforeEachSteps.length > 0) {
            ...
            it('Test case #1 should return errors ["email expected string,received number"] when body {"email":123}', async () => {
              testNumber = 1;
              totalTests++;
              const payloadObj = {"email":123};
              ...
              try {
                ...
                const expectJson = ["email expected string, received number"].sort();
            ...
            it('Test case #2 should return errors [] when body {"email":"ziichat@halome.com"}', async () => {
              testNumber = 2;
              totalTests++;
              const payloadObj = {"email":"ziichat@halome.com"};
              resolvedData = resolveVariables(payloadObj, globalContext);
            ...
            it('Test case #3 should return errors ["email required"] when body {}', async () => {
              testNumber = 3;
              totalTests++;
              const payloadObj = {};
              resolvedData = resolveVariables(payloadObj, globalContext);
              ...
              try {
                ...
                const data = response.data;
                const expectJson = ["email required"].sort();
            ...
            it('Test case #4 should return errors ["email should not be empty"] when body {"email":""}', async () => {
              testNumber = 4;
              totalTests++;
              ...
              try {
                ...
                const data = response.data;
                const expectJson = ["email should not be empty"].sort();
            ...
            it('Test case #5 should return errors ["email expected string,received null"] when body {"email":null}', async () => {
              testNumber = 5;
              totalTests++;
              const payloadObj = {"email":null};
              resolvedData = resolveVariables(payloadObj, globalContext);
              ...
              try {
                ...
                const data = response.data;
                const expectJson = ["email expected string, received null"].sort();
      ...
      afterEach(async () => {
          testCaseNumber++;
          const afterEachSteps = UpdateUserEmailRequest.options
         ...
         afterAll(async () => {
          const afterAllSteps = UpdateUserEmailRequest.options
  ...
  const fileName = 'update-user-email' + (chunkNumber ? `-chunk-undefined` : '') + '.result.json';
  const filePath = path.join(reportDir, fileName);
  fs.writeFileSync(filePath, JSON.stringify(testResult, null, 2), 'utf-8');

  console.log(`📝 Saved result for update-user-email chunk single to ${filePath}`);
    });
        })
