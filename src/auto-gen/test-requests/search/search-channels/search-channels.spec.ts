    ...
    import { SearchChannelsRequest } from '././search-channels.request';
    describe('Testcase for search-channels', () => {
        let totalTests = 0;
        let allSteps = [];
        let failedTests: any[] = [];
        let codedTest: any[] = [];
        ...
        beforeAll(async () => {
          ...
          const beforeAllSteps = SearchChannelsRequest.options
        ...
        beforeEach(async () => {
          testCaseNumber++;
          const beforeEachSteps = SearchChannelsRequest.options
          ...
          if (beforeEachSteps.length > 0) {
            ...
            it('Test case #1 should return errors ["keyword expected string,received number"] when body {"keyword":123}', async () => {
              testNumber = 1;
              totalTests++;
              const payloadObj = {"keyword":123};
            ...
            it('Test case #2 should return errors [] when body {"keyword":"channelname1"}', async () => {
              testNumber = 2;
              totalTests++;
              const payloadObj = {"keyword":"channelname1"};
              resolvedData = resolveVariables(payloadObj, globalContext);
            ...
            it('Test case #3 should return errors ["keyword required"] when body {}', async () => {
              testNumber = 3;
              totalTests++;
              const payloadObj = {};
              resolvedData = resolveVariables(payloadObj, globalContext);
            ...
            it('Test case #4 should return errors ["keyword should not be empty"] when body {"keyword":""}', async () => {
              testNumber = 4;
              totalTests++;
              const payloadObj = {"keyword":""};
              resolvedData = resolveVariables(payloadObj, globalContext);
            ...
            it('Test case #5 should return errors ["keyword expected string,received null"] when body {"keyword":null}', async () => {
              testNumber = 5;
              totalTests++;
              const payloadObj = {"keyword":null};
              resolvedData = resolveVariables(payloadObj, globalContext);
      ...
      afterEach(async () => {
          testCaseNumber++;
          const afterEachSteps = SearchChannelsRequest.options
         ...
         afterAll(async () => {
          const afterAllSteps = SearchChannelsRequest.options
  ...
  const fileName = 'search-channels' + (chunkNumber ? `-chunk-undefined` : '') + '.result.json';
  const filePath = path.join(reportDir, fileName);
  fs.writeFileSync(filePath, JSON.stringify(testResult, null, 2), 'utf-8');

  console.log(`📝 Saved result for search-channels chunk single to ${filePath}`);
    });
        })
