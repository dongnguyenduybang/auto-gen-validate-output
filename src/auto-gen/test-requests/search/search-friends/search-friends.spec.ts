    ...
    import { SearchFriendsRequest } from '././search-friends.request';
    describe('Testcase for search-friends', () => {
        let totalTests = 0;
        let allSteps = [];
        let failedTests: any[] = [];
        let codedTest: any[] = [];
        ...
        beforeAll(async () => {
          ...
          const beforeAllSteps = SearchFriendsRequest.options
        ...
        beforeEach(async () => {
          testCaseNumber++;
          const beforeEachSteps = SearchFriendsRequest.options
          ...
          if (beforeEachSteps.length > 0) {
            ...
            it('Test case #2 should return errors [] when body {"keyword":"testabc123"}', async () => {
              testNumber = 2;
              totalTests++;
              const payloadObj = {"keyword":"testabc123"};
              resolvedData = resolveVariables(payloadObj, globalContext);
      ...
      afterEach(async () => {
          testCaseNumber++;
          const afterEachSteps = SearchFriendsRequest.options
         ...
         afterAll(async () => {
          const afterAllSteps = SearchFriendsRequest.options
  ...
  const fileName = 'search-friends' + (chunkNumber ? `-chunk-undefined` : '') + '.result.json';
  const filePath = path.join(reportDir, fileName);
  fs.writeFileSync(filePath, JSON.stringify(testResult, null, 2), 'utf-8');

  console.log(`📝 Saved result for search-friends chunk single to ${filePath}`);
    });
        })
