...
export async function getSwagger() {
    if (!swagger) {
        const swaggerPath = path.resolve(__dirname, '../swagger/commands-chat.swagger.json');
...
export async function getOperationInfo(operationId: string) {
  ...
  export async function buildRequestFromAction(action: string, variables: any, header: any) {
