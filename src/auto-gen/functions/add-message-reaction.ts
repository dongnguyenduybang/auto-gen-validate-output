...
export async function addMessageReaction(token: string, workspaceIdField: string, channelIdField: string, messageIdField: string, emojiField: string) {
  try {
      if (!token) {
      ...
      const payload = { workspaceId: workspaceIdField , channelId: channelIdField, messageId: messageIdField, emoji: emojiField };
