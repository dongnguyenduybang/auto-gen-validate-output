...
import { validateSendLocation } from '../validates/send-location/validate-send-location';
import fs from 'fs';
...
import { SendLocationResponse } from '../dto-response/send-location.response';
import { validateAfterLogic } from '../validates/send-location/validate-send-location-after';

describe('Testcase for send-location', () => {
  let totalTests = 0;
  let passedLogic = 0;
  let failedTests = [];
  let logicTests = [];
  ...
  it('Test case # 1296 with expect errors [] ', async () => {
    testNumber = 1296;
    totalTests++;
    const payloadObj = { "workspaceId": "0", "channelId": "{{channelId}}", "latitude": "10.03273", "longitude": "105.7834439", "ref": "ref", "content": "📍 Location" };
    resolvedData = resolveJsonVariables(payloadObj);

    const requestUrl = `${globalThis.url}/Message/SendLocation`;
    ...
    try {
      ...
      if (response.status === 201) {
        ...
        const dtoInstance = plainToClass(SendLocationResponse, data);
        const validateLogic = await validateSendLocation(dtoInstance, resolvedData);
  ...
  afterAll(async () => {
    ...
    const folderPathLogic = path.join(__dirname, '../reports/send-location');
    ...
    const resultContent = `
=== Test Reports for DTO "send-location" ===
Host: ${globalThis.url}
...
${failedTests.map((failCase) => `
  - Testcase #${failCase.testcase}
    ...
    === Test Reports Logic for DTO "send-location" ===
    Host: ${globalThis.url}
    Endpoint: /Message/SendLocation
    Error: 
  ...
  ` ).join('')}`
    ...
    const resultFilePath = path.join(folderPath, 'send-location.txt');
    const resultFilePathLogic = path.join(folderPathLogic, `send-location.${getTime()}.txt`);
