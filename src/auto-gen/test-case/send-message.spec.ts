...
describe('Testcase for send-message', () => {
  ...
  beforeAll(async () => {
    ...
    const beforeStep = await executeBeforeAllSteps([
      ...
      "getChannel(body: {workspaceId: '0', channelId: '{{createChannel.channelId}}'}, header: {token: '{{mockUser.token}}'}, expect: {countMember: 1, countMessage: 1, isChannelId: '{{createChannel.channelId}}',isLastMessage: '{{createChannel.messageId}}', isContent: '%s created this channel', isOwner: '{{mockUser.userId}}'})",
    ]);
    failedStep.push({
      error: beforeStep
    });
