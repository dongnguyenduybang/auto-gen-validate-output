    ...
    describe('Testcase for mock-channel', () => {
        ...
        afterEach(async () => {
            ...
            const result = await executeBeforeAllSteps(["getChannel({{token}}, {{channelId}})"])
            ...
            it('Test case # 562 with expect errors [] ', async () => {
             testNumber = 562;
              ...
              const requestUrl = `${globalThis.url}/InternalFaker/MockChannels`;
            ...
            try {
              ...
              if(response.status === 201){
                  ...
                  const validateLogic = await validateMockChannel(dtoInstance, resolvedData);
      ...
      afterAll(async () => {
        ...
        const folderPathLogic = path.join(__dirname, '../reports/mock-channel');
...
${failedTests.map((failCase) => `
  - Testcase #${failCase.testcase}
    ...
    === Test Reports Logic for DTO "mock-channel" ===
    Host: ${globalThis.url}
    Endpoint: /InternalFaker/MockChannels
    Error: 
...
const resultFilePathLogic = path.join(folderPathLogic, `mock-channel.${getTime()}.txt`);
