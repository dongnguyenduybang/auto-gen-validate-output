...
describe('Template testcase', () => {
    ...
    beforeAll(async () => {
        ...
        const requestPath = 'src/auto-gen/dtos/send-message/send-message.request.json'
        const requestConfig = readJsonFile(requestPath);
        console.log(requestConfig)
        executeBeforeAllSteps(requestConfig.beforeAll)

        headers = { "Content-Type": "application/json", "x-session-token": '{{token}}', "x-country-code": "VN" }
    })
