    ...
    import { validateDeleteMessageForEveryone } from '../validates/delete-message-for-everyone/validate-delete-message-for-everyone';
    import fs from 'fs';
    ...
    import { DeleteMessageForEveryoneResponse } from '../dto-response/delete-message-for-everyone.response';
    import { validateAfterLogic } from '../validates/send-location/validate-send-location-after';

    describe('Testcase for delete-message-for-everyone', () => {
        let totalTests = 0;
        let passedLogic = 0;
        let failedTests = [];
        let logicTests = [];
          ...
          if (messageId) {
            ...
            it('Test case # 1 with expect errors ["workspaceId should not be empty","channelId should not be empty","messageId should not be empty"] ', async () => {
             testNumber = 1;
              ...
              const requestUrl = `${globalThis.url}/Message/DeleteMessageForEveryone?${urlParams}`;
            ...
            try {
              ...
              if(response.status === 201){
                  ...
                  const dtoInstance = plainToClass(DeleteMessageForEveryoneResponse, data);
                  const validateLogic = await validateDeleteMessageForEveryone(dtoInstance, resolvedData);
      ...
      afterAll(async () => {
        ...
        const folderPathLogic = path.join(__dirname, '../reports/delete-message-for-everyone');
        ...
        const resultContent = `
=== Test Reports for DTO "delete-message-for-everyone" ===
Host: ${globalThis.url}
Endpoint: /Message/DeleteMessageForEveryone
Summary: 
...
${failedTests.map((failCase) => `
  - Testcase #${failCase.testcase}
    ...
    === Test Reports Logic for DTO "delete-message-for-everyone" ===
    Host: ${globalThis.url}
    Endpoint: /Message/DeleteMessageForEveryone
    Error: 
...
const resultFilePath = path.join(folderPath, 'delete-message-for-everyone.txt');
const resultFilePathLogic = path.join(folderPathLogic, `delete-message-for-everyone.${getTime()}.txt`);
