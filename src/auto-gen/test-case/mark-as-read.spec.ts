    ...
    import { validateMarkAsRead } from '../validates/mark-as-read/validate-mark-as-read';
    import fs from 'fs';
    ...
    describe('Testcase for mark-as-read', () => {
          ...
          if (messageId) {
            ...
            it('Test case # 1 with expect errors ["workspaceId should not be empty","channelId should not be empty","messageId should not be empty"] ', async () => {
             testNumber = 1;
              ...
              const requestUrl = `${globalThis.url}/Message/MarkAsRead`;
            ...
            try {
              ...
              if(response.status === 201){
                  ...
                  const dtoInstance = plainToClass(MarkAsReadResponse, data);
                  const validateLogic = await validateMarkAsRead(dtoInstance, resolvedData);
      ...
      afterAll(async () => {
        ...
        const folderPathLogic = path.join(__dirname, '../reports/mark-as-read');
        ...
        const resultContent = `
=== Test Reports for DTO "mark-as-read" ===
Host: ${globalThis.url}
Endpoint: /Message/MarkAsRead
Summary: 
...
${failedTests.map((failCase) => `
  - Testcase #${failCase.testcase}
    ...
    === Test Reports Logic for DTO "mark-as-read" ===
    Host: ${globalThis.url}
    Endpoint: /Message/MarkAsRead
    Error: 
...
const resultFilePath = path.join(folderPath, 'mark-as-read.txt');
const resultFilePathLogic = path.join(folderPathLogic, `mark-as-read.${getTime()}.txt`);
