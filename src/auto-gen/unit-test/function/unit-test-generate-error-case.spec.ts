import { combineFields, generateCombinations, generateErrorVariantsForField, getDecorators, softErrorFromMap } from "../../utils/dto-helper";
...
describe('Unit test for function generalErrorCase()', () => {
    ...
    describe('Unit test for function combineFields()', () => {
        ...
        it("should return all combineFields for 'workspaceId' and 'content' field ", () => {
            ...
            expect(result).toContainEqual([{ workspaceId: undefined }, { content: undefined }]);
            expect(result).toContainEqual([{ workspaceId: '' }, { content: 'a'.repeat(2001) }]);
            expect(result).toContainEqual([{ workspaceId: 'invalid' }, { content: '' }]);
            expect(result).toContainEqual([{ workspaceId: '0' }, { content: undefined }]);
        })
    });
    ...
    describe('Unit test for function softErrorFromMap()', () => {
        ...
        describe('Priority error case', () => {
            ...
            it("should return 'Unauthorized request' for invalid value 'userId'", () => {
                const payload = { workspaceId: '0', channelId: '{{channelId}}', userId: 'invalid' };
        ...
        describe('No priority error case', () => {
            it("should return 'must have required property ref' for undefined 'ref'", () => {
                const payload = {workspaceId: '0', channelId: '{{channelId}}', content: 'content', ref: undefined};
                const result = softErrorFromMap(payload, TestDto);
                expect(result).toEqual(["must have required property 'ref'"]);
            });
            it("should return 'ref must have more than 1 length' for empty 'ref'", () => {
                const payload = {workspaceId: '0', channelId: '{{channelId}}', content: 'content', ref: ''};
                const result = softErrorFromMap(payload, TestDto);
            ...
            it("should return 'ref must have more than 1 length, content must to range from 1 to 2000 length' for empty 'ref' and 'content'", () => {
                const payload = {workspaceId: '0', channelId: '{{channelId}}', content: '', ref: ''};
                const result = softErrorFromMap(payload, TestDto);
                expect(result).toEqual(["ref must have more than 1 length", "content must to range from 1 to 2000 length"]);
            });
        });
    });
